...
function layoutFor(format: CatalogFormat): Layout {
  const cfg = LAYOUT_CONFIGS[format];
  if (!cfg) {
    return { columns: 2, rowsFirstPage: 4, rowsPerPage: 5, imageHeightMm: 36, cardHeightMm: 57, compact: false };
  }
  ...
  const rowsPerPage = cfg.rowsPerPage
    ?? Math.max(1, Math.floor(PAGE_HEIGHT_MM / rowHeight));
...
function renderProfile(profile?: PrintableProfile) {
  if (!profile) return '';
...
function renderProductCard(product: PrintableProduct, cardWidth: string, simpleList: boolean) {
...
export class StandardGridCatalogRenderer implements CatalogRenderer {
  readonly format: CatalogFormat = 'grid-2';
  ...
  render(
    ...
    const body = renderBody(input, productsForPdf, profileHtml, headerHtml, layout);
...
function renderBody(
  ...
  while (rowIdx < allRows.length) {
    ...
    for (const row of chunk) {
      const cards = row
        .map((p) => renderProductCard(p, cardWidth, input.format === 'simple-list'))
        .join('');
